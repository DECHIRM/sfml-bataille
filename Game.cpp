...
void DrawFields(player& p, Sprite** tab, bool visibility, RenderWindow& window, int x, int y, double scale, bool canSelect, bool online, field** tab_enemy)
{
	...
	for (int i = 0; i < SIZE_OF_PG; i++)
		for (int j = 0; j < SIZE_OF_PG; j++)
		{
			...
			tmp.setScale(float(scale), float(scale));
			tmp.setPosition(x + float(scale) *100*j, y + float(scale) *100*i);
			window.draw(tmp);
		}
}
...
void Game::menu()
{
	...
	for (int i = 0; i < nOptions; i++)
	{
		...
		options[i].setPosition(cfg::WINDOW_WIDTH / 2 - options[i].getLocalBounds().width / 2, 200 + i * float(100));
...
void Game::Play(player& p, player& opponent, string s)
{
	...
	title_me.setPosition((cfg::first_pg_x + 500* float(cfg::scale)) - title_me.getGlobalBounds().width / 2, cfg::first_pg_y + 1000* float(cfg::scale));
	title_opp.setPosition((cfg::second_pg_x + 500 * float(cfg::scale)) - title_opp.getGlobalBounds().width / 2, cfg::second_pg_y + 1000 * float(cfg::scale));
	back_button.setPosition(cfg::game_window_width - float(cfg::scale) * 100 - back_button.getGlobalBounds().width, float(cfg::scale) * 50);
	communicate.setPosition(float(cfg::scale) * 100, float(cfg::scale) * 50);
